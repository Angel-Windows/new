...
@section('main')
    ...
    @foreach(config('translatable.locales') as $locale)
        <div class="form-group">
            <div class="col-sm-2 control-label" style="padding-top:0!important;"><label style="margin-right:10px;">Название-{{$locale}}</label>
            </div>
            <div class="col-sm-10">
    ...
    @foreach(config('translatable.locales') as $locale)
        <div class="form-group">
            <div class="col-sm-2 control-label" style="padding-top:0!important;"><label
                        style="margin-right:10px;">Текст -{{$locale}}</label></div>
            <div class="col-sm-10">
    ...
    @foreach(config('translatable.locales') as $locale)
        <div class="form-group">
            <div class="col-sm-2 control-label" style="padding-top:0!important;"><label style="margin-right:10px;">Текст
                    ссылки(если оставить пустым будет "Подробнее")-{{$locale}}</label></div>
            <div class="col-sm-10">
{{--                {!! Form::text('textButton',  '', array('class'=>'form-control') ) !!}--}}
                <input type="text" class="form-control" id=""
                       name="{{$locale}}[textButton]"
                       value="">
            </div>
        </div>
    @endforeach
        ...
        <div class="row">
            @foreach ( $slides as $slide )
                <div class="col-sm-6 col-md-2">
                    <div class="thumbnail">
                        {!! Form::hidden('title', $slide->translate('uk')->title ?? '') !!}
                        {!! Form::hidden('text', $slide->text) !!}
                        {!! Form::hidden('slide_id', $slide->id) !!}
                        {!! Form::hidden('link', $slide->link) !!}

                        <img style="width: 100%;" alt="photo" src="/uploads/slides/{{{ $slide->image }}}"
{{--                             data-toggle="modal"--}}
                        ...
                        <div class="caption">
                            <div class="btn-group">
                                ...
                                <button title="Удалить" type="button" class="delete btn btn-danger"
                                        data-id="{{ $slide->id }}"><span class="glyphicon glyphicon-remove"></span>
                                </button>
                            </div>
                        </div>
                    </div>
