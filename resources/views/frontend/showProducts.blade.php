        ...
        <div class="container">
            <div class="row">
                <div class="leftBar col-lg-3">
                    <div class="title">{{trans('cosmetics.filter')}}</div>
                    @if( Input::has('filter') )
                        <div class="selectedFeatures">
                            ...
                            <a href="#" id="clear_all_filters" class="button">{{trans('cosmetics.reset')}}</a>
                        </div>
                    @endif
                    ...
                    <form id="filter" method="get">
                        <div class="priceChange">
                            ...
                            <div id="polzunok"></div>
                            <div id="price-range" class="price_range" data-max="{{ $price_range['max']}}"
                                 data-min="{{ $price_range['min']}}"></div>
                            ...
                            @if( count( $f->options ) )
                                <div class="featureBlock">
                                    ...
                                    <div class="fcontent">
                                        @foreach( $f->options as $key => $o )
                                            <div class="item">
                                                <input @if( isset( Input::get('filter')[$f->id] )  && in_array( $o->value, Input::get('filter')[$f->id] ) ) checked
                                                       @endif onchange="this.form.submit()"
                                                       name="filter[{{ $f->id }}][]" type="checkbox"
                                                       class="filterCheckbox" value="{{{ $o->value }}}"
                                                       id="{{ $f->id.$o->id }}">
                                                <label for="{{ $f->id.$o->id }}">{{ $o->value }} ({{ $o->count }}
                                                    )</label>
                                            </div>
                                        @endforeach
                                        @if(count($f->options) > 4)
                                            <a data-show="1" href="#"
                                               class="button seeMoreFeature">{{trans('cosmetics.show_more')}} </a>
                                        @endif
                                    </div>
                                </div>
                            @endif
                ...
                <div class="rightBar col-lg-9">
                    <div class="row">
                        <div class="topSettings col-xs-12">
                            <div class="featureButton" id="featureButton">{{trans('cosmetics.filter')}}</div>
                            <div class="sorting">
                                <span class="text">{{trans('cosmetics.sort_from')}}:</span>
                                <form action="#" method="GET" class="formSort">
                                    <select onchange="this.form.submit()" name="sorter" id="sort">
                                        ...
                                        <option value="ASC_price"
                                                @if(Input::get('sorter') == 'ASC_price') selected @endif>{{trans('cosmetics.price_up')}}</option>
                                        <option value="DESC_price"
                                                @if(Input::get('sorter') == 'DESC_price') selected @endif>{{trans('cosmetics.price_down')}}</option>
                                        <option value="ASC_abc"
                                                @if(Input::get('sorter') == 'ASC_abc') selected @endif>{{trans('cosmetics.price_alphabet_up')}}</option>
                                        <option value="ASC_cba"
                                                @if(Input::get('sorter') == 'ASC_cba') selected @endif>{{trans('cosmetics.price_alphabet_down')}}</option>
                                    </select>
                                </form>
                            </div>

                            <div class="showBlock">
                                <span class="text">{{trans('cosmetics.vid')}}:</span>
                                <div class="how">
                                    <div data-show="1" id="plitka"
                                         class="plitka @if(Session::get('show') == 0) active @endif"></div>
                                    <div data-show="0" id="list"
                                         class="list @if(Session::get('show') == 1) active @endif"></div>
                                </div>
                            </div>
                        ...
                        <!--Список-->
                        <div id="productList" @if(Session::get('show') == 1) style="display: block;"
                             @else style="display: none" @endif class="listProducts col-xs-12">
                                        ...
                                        <div class="item @if($prod->discount == 1) discount @endif @if($prod->new == 1) new @endif @if($prod->sale == 1) sale @endif">
                                            <div class="row">
                                                <div class="image col-lg-4 col-md-5 col-sm-5">
                                                    @if( $prod->image !='')
                                                        <img src="/uploads/products/md/{{ $prod->image }}"
                                                             alt="{{ $prod->name }}">
                                                    @else
                                                        <img src="{{asset('uploads/products/md/no_image.png')}}"
                                                             alt="#">
                                                    @endif
                                                </div>
...
{{--                                                    @include('partials.charlist')--}}
                                                    ...
                                                    <div class="infBottom">
                                                        ...
                                                        <div class="price">
                                                            ...
                                                            @endif
                                                            <div class="now">
                                                                <span>{{ number_format($prod->price - ($prod->price * $userDiscount), 2, ',', '') }}</span>
                                                                Грн
                                                            </div>
                                                        </div>
                        ...
                        <!--сетка-->
                        <div id="productGrid" @if(Session::get('show') == 0) style="display: block"
                             @else style="display: none" @endif class="products col-xs-12">
                            <!--<div class="products">-->
                            @if(count($products) > 0)
                                @foreach( $products as $prod)
                                            ...
                                            <div class="image">
                                                @if( $prod->image !='')
                                                    <img src="/uploads/products/md/{{ $prod->image }}"
                                                         alt="{{ $prod->name }}">
                                                @else
                                                    <img src="{{asset('uploads/products/md/no_image.png')}}" alt="#">
                                                @endif
                                            </div>
                        ...
                        @if(isset($lastSeenProducts) && count($lastSeenProducts) > 0)
                            ...
                            <div class="lastSeenBlock col-xs-12">
                                <div class="ltitle">{{trans('cosmetics.see_more')}}</div>
                                    ...
                                    @foreach($lastSeenProducts as $item)
                                        ...
                                        <div class="product col-lg-4 col-md-4 col-sm-4 col-xs-12">
                                            <div class="item @if($item->discount == 1) discount @endif @if($item->new == 1) new @endif @if($item->sale == 1) sale @endif">
                                                <div class="image">
                                                    @if( $item->image !='')
                                                        <img src="/uploads/products/md/{{ $item->image }}"
                                                             alt="{{ $item->name }}">
                                                    @else
                                                        <img src="{{asset('uploads/products/md/no_image.png')}}"
                                                             alt="#">
                                                ...
                                                <div class="information">
                                                    ...
                                                    <div class="prices">
                                                        ...
                                                        @endif
                                                        <div class="now">
                                                            <span>{{ number_format($item->price - ($item->price * $userDiscount), 2, ',', '') }}</span>
                                                            Грн
                                                        </div>
                                                    </div>
                                ...
                                <div class="row">
                                    <div class="col-xs-12">
                                        <a href="/category/allseen" id="seeMore"
                                           class="seeMore">{{trans('cosmetics.see_all')}}</a>
                                    </div>
                                </div>
                            </div>
...
@section('styles')
    <link rel="stylesheet" href="{{asset('frontend/css/contactLenses.css')}}">
    <link rel="stylesheet" href="{{asset('frontend/js/slick/slick-theme.css')}}">
    <link rel="stylesheet" href="{{asset('frontend/js/slick/slick.css')}}">
@endsection

@section('scripts')
    <!--formStyler-->
    <script type="text/javascript" src="{{asset('frontend/js/formStyler/jquery.formstyler.min.js')}}"></script>
    <link rel="stylesheet" href="{{asset('frontend/js/formStyler/jquery.formstyler.css')}}">
    <link rel="stylesheet" href="{{asset('frontend/js/formStyler/jquery.formstyler.theme.css')}}">
    ...
    <script>
        var shownews = "{{ trans('cosmetics.show_more') }}";
        var hidenews = "{{ trans('cosmetics.filter_hide') }}";
    </script>
    <script src="{{asset('frontend/js/polzunok.js')}}"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jqueryui-touch-punch/0.2.3/jquery.ui.touch-punch.min.js"></script>
    <script src="{{asset('frontend/js/contactLenses.js')}}"></script>
@endsection