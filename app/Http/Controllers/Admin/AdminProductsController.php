...
class AdminProductsController extends AdminBaseController
{
    ...
    public function getIndex()
    {
        ...
        $products = $products->withTranslation()->paginate(15);

        $categories = Category::getNestedList('name', null, '&nbsp;&nbsp;&nbsp;');
        $categories = array('all' => 'Все') + $categories;


        $products_all = Products::withTranslation()->get();
        ...
        return view('admin.showProducts',
            compact(['title', 'products', 'products_search', 'products_count', 'categories', 'marks', 'sortings']));
    }
    ...
    public function postIndex(Request $request)
    {
        ...
        if ($request->get('action') == 'delete') {
            ...
            $translations=ProductsTranslation::where('products_id', $ids)->get();
    ...
    public function getAdd()
    {
        ...
        return view('admin.editProductsPost',
            compact(['post', 'title', 'categories', 'prod_page', 'features', 'alike_prod', 'charlist']));
    }
    ...
    public function postAdd(AddProductRequest $request)
    {
        ...
        $input = array_merge($request->except('categories', 'feature', 'row_table', 'alike', 'charlist'),
            ['alike' => serialize($alike_prod)]);
    ...
    public function getEdit($id)
    {
        ...
        $features = Features::withTranslation()->with('variants')->get();
        $options = Options::getOptions($id);

        $charlist = Charlist::withTranslation()->with('variants')->get();
        ...
        $alike_prod = Products::withTranslation()->where('id', '!=', $post->id)->get();
        $post->alike = ($post->alike != '') ? unserialize($post->alike) : [];


        return view('admin.editProductsPost', compact([
            'post', 'title', 'categories', 'prod_page', 'features', 'options', 'alike_prod', 'charlist',
            'charlist_options'
        ]));
    }
    ...
    {
        $alike_prod = $request->has('alike') ? $request->get('alike') : [];
        $input = array_merge($request->except('categories', 'feature', 'row_table', 'charlist'),
            ['alike' => serialize($alike_prod)]);
