...
class AdminStaffController extends AdminBaseController
{
    ...
    public function productsManagement(){
        ...
        $title = 'Управление';
        return view('admin.editStaff', compact('title'));
    }
    ...
    public function getIndex(){
        ...
        if( $product_id = Input::get('product') ){
            if($product_id == 'all'){
                ...
                $products = $products->whereName($product_id); // исправление для правильного отображения всех записей
            }
        }
        ...
        foreach ($products_all as $prod) {
            $products_search[$prod->name] = $prod->name; // исправление для правильного отображения всех скопированных полей
        }
        if( count($products) > 0 )
        ...
        return view('admin.editStaff', compact(['title', 'products', 'products_search', 'products_count', 'categories',  'marks', 'sortings']));
    }
...
//        print_r( $request->get('charlist'));exit();
    ...
    // New functions for Copy Products ( функция для копирования записей )
