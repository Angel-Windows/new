...
class BaseController extends Controller
{
    ...
    {
        $pages = Page::trans()->get();
        $categories = Category::trans()->get()->toHierarchy();
...
//            print_r($user);exit();
        ...
        $categoriesFoot = Category::trans()->where('parent_id', null)->take(3)->get()->toArray();

        $pageFoot = Page::trans()->whereType('main')->skip(1)->take(2)->get()->toArray();
        ...
        view()->share(compact([
            'compare_ids', 'pages', 'settings', 'categories', 'cart_info', 'footCatArray', 'googleAuthLink',
            'facebookAuthLink', 'userDiscount'
        ]));
    }
    ...
    public function sendRewiew(Request $request)
    {
        ...
        Products::whereId($product_id)->update([
            'votes' => ($product->votes + 1), 'rating_sum' => ($product->rating_sum + $starsCount)
        ]);
