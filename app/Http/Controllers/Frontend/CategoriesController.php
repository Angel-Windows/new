...
class CategoriesController extends BaseController
{
    ...
    public function getProducts(Request $request, $slug)
    {
        if ($slug == 'allseen') {
            ...
            $breadcrumb->name = trans('cosmetics.last_product');
            ...
            $page = new Page;
            $page->name = $page->meta_title = trans('cosmetics.last_product');
        ...
        $page = Category::trans()->whereSlug($slug)->first();
        ...
        $products = Products::trans()->Join('dv_products_categories', 'dv_products.id', '=',
        ...
        $products = $products->where('price', '!=', '0,00')->latest('id')->paginate($per_page);
...
// print_r($features);exit;
        ...
        if (Session::get('product_toggle')) {
            $toggle = Session::get('product_toggle');
        } else {
            $toggle = 0;
        }
        ...
        if (is_array($seenArray)) {
            $lastSeenProducts = Products::trans()->whereIn('id', $seenArray)->get();
            $lastSeenProducts = Products::refImg($lastSeenProducts);
        }


        return view('frontend.showProducts', compact([
            'breadcrumbs', 'parent_cat', 'page', 'products', 'features', 'toggle', 'per_page_opt', 'cur_price',
            'price_range', 'next', 'prev', 'url', 'lastSeenProducts'
        ]));
    ...
    {
        $features = Features::trans()->groupBy('id')->orderBy('name')->get();
        ...
        foreach ($features as $key => $value) {
            $options = Options::join('dv_products_categories', 'dv_options.product_id', '=',
                'dv_products_categories.product_id')
                ->whereIn('dv_products_categories.category_id', $child_ids)
                ...
                ->get();
            $count = 0;
            $productsArray = Products::trans()->join('dv_products_categories', 'dv_products.id', '=',
                'dv_products_categories.product_id')
                ->whereIn('dv_products_categories.category_id', $child_ids)
    ...
    private function priceRange($child_ids)
    {
        ...
        $price_range = Products::trans()->leftJoin('dv_products_categories', 'dv_products.id', '=',
            'dv_products_categories.product_id')
            ->whereIn('dv_products_categories.category_id', $child_ids)
