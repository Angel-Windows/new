...
class PageController extends BaseController
{
	public function index( $page_url ){
		...
		$page = Page::trans()->whereSlug( $page_url )->firstOrFail();

		return view('frontend.showPage', compact(['page']));
	}

	public function getFavorites(){
		$page = Page::trans()->whereSlug( 'favorites' )->firstOrFail();
        // избранное
    ...
    public function getBrand( $page_url ){
        $page = Brands::trans()->whereSlug( $page_url )->firstOrFail();

        return view('frontend.showPage', compact(['page']));

    }
    public function getContacts(){
        $page = Page::trans()->whereId(40)->firstOrFail();
