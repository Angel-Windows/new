...
class ProductController extends BaseController
{
    public function showProduct($slug)
    {
        ...
        $page = Products::trans()->whereSlug($slug)->firstOrFail();
        $page->specification = unserialize($page->specification);
        $breadcrumbs = Category::get_parent_categories(ProductsCategories::where('product_id', '=',
            $page->id)->pluck('category_id'));
...
//		print_r($prod_reviews);exit();
        ...
        $alike_pr = Products::trans()->whereIn('id', unserialize($page->alike))->get();
        ...
        $charlist = Charlist::trans()->with('variants')->get();
        ...
        return view('frontend.showProduct', compact('page', 'alike_pr', 'breadcrumbs',
            'prod_reviews', 'features', 'offers', 'charlist', 'charlist_options'));
    }
    ...
    public function postAddReviews(AddProductReviewRequest $request)
    {
        ...
        return redirect()->back()->with('rev_message',
            'Ваш отзыв успешно добавлен и будет отображен после проверки администратором');
    }
