...
class PersonalAreaController extends BaseController
{
    public function index()
    {
        ...
        $page = Page::trans()->whereSlug('personal-area')->first();

        if (Auth::check()) {
            $user = Auth::user();

            $orders = Orders::with([
                'orderedproducts' => function ($query) {
                    $query->leftJoin('dv_products', 'dv_orders_products.product_id', '=', 'dv_products.id')
                        ->select([
                            'dv_orders_products.*', 'dv_products.imgs', 'dv_products.slug', 'dv_products.name',
                            'dv_products.main_img'
                        ])
                        ->get();
                }
            ])
    ...
    public function getRegister()
    {
        ...
        $page = new Page;
        $page->meta_title = trans('cosmetics.cabinet.registered');
    ...
    public function notifications()
    {
        ...
        $page = new Page;
        $page->meta_title = $page->name = trans('cosmetics.cabinet.notify');
    ...
    public function postNotifications(Request $request)
    {
        ...
        $user = Auth::user();
        Notification::create([
            'delivery_date' => $request->get('delivery_date'), 'interval' => $request->get('interval'),
            'repeat' => $repeat, 'user_id' => $user->id, 'last_send' => $request->get('delivery_date')
        ]);
...
//        print_r($request->all());exit();
    ...
    public function getEditInfo()
    {
        ...
        if ($user = Auth::user()) {
            ...
            $page = Page::trans()->whereSlug('edit-info')->first();
