...
class Products_old extends Model
{
//    use Translatable;
...
//    ];
	...
	public function category() {
		return $this->hasOne( 'App\Models\Category' , 'product_id' );
	}
	...
	public static function saveImgs( $request, $id ){
		...
		foreach( $request->file('image') as $file ) {
            $img = Image::make( $file );
            $filename = time() . '_' . $id . str_replace('image/', '.', $img->mime);

			$img->widen(500)->save( 'uploads/products/lg/' . $filename, 100 );
			$img->widen(290,null)->save( 'uploads/products/md/' . $filename);
