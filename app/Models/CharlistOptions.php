...
use App\Facade\TranslateBranch;
    ...
    public static function getOptions($id)
    {
        ...
        foreach ($options_all as $option) {
            if (isset($options[$option->charlist_id])) {
                $options[$option->charlist_id][] = (app()->getLocale() == 'uk') ? TranslateBranch::getTranslateText($option->value,
                    'uk') : $option->value;
            } else {
                $options[$option->charlist_id] = [
                    (app()->getLocale() == 'uk') ? TranslateBranch::getTranslateText($option->value,
                        'uk') : $option->value
                ];
            }
        }
